...
class Vector:
    """Class mostly from https://gist.github.com/mcleonard/5351452"""
    ...
    def angle_between(self, vector: "Vector") -> float:
        if not isinstance(vector, Vector):
            raise ValueError("The dot product requires another vector")
        angle = math.degrees(
            math.acos(self.dot(vector) / (self.norm() * vector.norm()))
        )
        return angle

    def distance(self, vector: "Vector") -> float:
        if not isinstance(vector, Vector):
            raise ValueError("The distance function requires another vector")
        distance = math.sqrt(
            abs(sum(self.values[i] - vector.values[i] for i in range(len(self.values))))
        )
        return distance
    ...
    def matrix_mult(self, matrix: list[list]):
